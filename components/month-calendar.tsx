...
export function MonthCalendar({ userId }: MonthCalendarProps) {
  ...
  const [absenceType, setAbsenceType] =
    useState<'vacation' | 'medical' | 'personal' | 'other'>('vacation')
  ...
  const loadDayEntries = async (dateStr: string) => {
    ...
    try {
      // 1) YYYY-MM-DD -> fechas de inicio / fin de día
      ...
      usersSnap.docs.forEach((docSnap) => {
        ...
        const uid = (data.uid as string) || docSnap.id
        userMap.set(uid, (data.displayName as string) || 'Usuario')
      })

      // 4) Construir entradas
  ...
  const handleSaveAbsence = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'absences'), {
        userId,
        userName,          // 👈 nombre guardado
        userEmail,         // 👈 email guardado (opcional)
        type: absenceType,
          ...
          <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
            ...
            <div className="space-y-3 border-t pt-4">
              ...
              <div>
                ...
                <Select
                  value={absenceType}
                  onValueChange={(v) => setAbsenceType(v as any)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <Button
                ...
                disabled={saving || !absenceEndDate}
                className="w-full"
              >
        ...
        {selectedDate && (
          <div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg space-y-3">
            <h4 className="font-semibold text-sm">
              Horas ingresadas para esta fecha:
            </h4>
            {loadingEntries ? (
              ...
              <div className="space-y-2">
                {dayEntries.map((entry, idx) => (
                  <div
                    key={idx}
                    className="text-sm p-2 bg-white dark:bg-slate-900 rounded border"
                  >
